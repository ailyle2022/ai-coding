<template>
  <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
    <!-- 添加/编辑用户模态框 -->
    <EditUserModal v-if="showEditModal" :user="editingUser" @close="showEditModal = false" @saved="handleUserSaved" />
    ...
    <table class="min-w-full divide-y divide-gray-200">
      <thead class="bg-gray-50">
        <tr>
          <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider rounded-tl-xl">{{
            $t('users.username') }}</th>
          <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">{{
            $t('users.email') }}</th>
          <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">{{
            $t('users.name')}}</th>
          <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">{{ $t('users.role')
            }}</th>
          <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">{{
            $t('common.status')}}</th>
        ...
        <tr v-for="user in users" :key="user.id">
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm">
            <span class="px-3 inline-flex text-sm leading-5 font-semibold rounded-full"
              ...
              {{ user.isActive ? $t('common.enable') : $t('common.disable') }}
            </span>
          </td>
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
            <button @click="editUser(user)" class="text-primary-accent hover:text-blue-900 mr-4 text-sm">
              {{ $t('common.edit') }}
            </button>
